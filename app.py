...
# ========= AUTH =========
from routes.auth_routes import setup_auth_routes, SESSION_COOKIE, _get_session_cookie
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. Based code's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-BasedCode-Owner, attribute the request to that user only
...
async def _startup_event():
    ...
    # `BASEDCODE_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("BASEDCODE_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (BASEDCODE_INPROCESS_TASKS=0); "
