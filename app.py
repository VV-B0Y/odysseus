...
app.add_middleware(
    ...
    allow_headers=[
        ...
        "X-API-Key",
        "X-Auth-Token",
        "X-BasedCode AI-Internal-Token",
        "X-BasedCode AI-Owner",
        "X-Requested-With",
        "X-TZ-Offset",
    ],
)
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. BasedCode AI's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-BasedCode AI-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-BasedCode AI-Owner") or "").strip()
...
async def _startup_event():
    ...
    # `BASEDCODE_AI_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = os.environ.get("BASEDCODE_AI_INPROCESS_TASKS", "1").strip().lower()
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (BASEDCODE_AI_INPROCESS_TASKS=0); "
