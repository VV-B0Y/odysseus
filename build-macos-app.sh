#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for BasedCode AI.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/BasedCode AI.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/BasedCode AI.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with BASEDCODE_AI_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="BasedCode AI"
INSTALL_DIR="$REPO_DIR"
PORT="${BASEDCODE_AI_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — center-crop docs/basedcode-ai.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/basedcode-ai.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/basedcode-ai.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/basedcode-ai.jpg" "$TMPIMG/sq.png"
  sips -z 512 512 "$TMPIMG/sq.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/basedcode-ai.icns" >/dev/null 2>&1; then
    echo "  icon:        basedcode-ai.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no docs/basedcode-ai.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.basedcode-ai.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>basedcode-ai</string>
...
#!/bin/bash
# BasedCode AI.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/basedcode-ai-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"BasedCode AI\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"BasedCode AI\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "BasedCode AI isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "BasedCode AI failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "BasedCode AI is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag BasedCode AI to Applications)"
