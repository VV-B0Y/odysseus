#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Based code.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Based code.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Based code.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with BASEDCODE_PORT
# (legacy: ODYSSEUS_PORT).
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="Based code"
INSTALL_DIR="$REPO_DIR"
PORT="${BASEDCODE_PORT:-${ODYSSEUS_PORT:-7860}}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — center-crop docs/basedcode.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/basedcode.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/basedcode.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/basedcode.jpg" "$TMPIMG/sq.png"
  sips -z 512 512 "$TMPIMG/sq.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/basedcode.icns" >/dev/null 2>&1; then
    echo "  icon:        basedcode.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no docs/basedcode.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.basedcode.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>basedcode</string>
...
#!/bin/bash
# Based code.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/basedcode-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Based code\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Based code\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Based code isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Based code failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Based code is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Based code to Applications)"
