---
name: basedcode-ai
description: Use when the user asks Claude Code to read or write BasedCode AI data (todos, email, calendar, memory, documents) through the scoped Claude Agent API. Requires BASEDCODE_AI_URL and BASEDCODE_AI_API_TOKEN.
---

# BasedCode AI

Use this skill when a user asks to interact with BasedCode AI from Claude Code.

## Configuration

Expect these environment variables:

- `BASEDCODE_AI_URL`: Base URL for the user's BasedCode AI instance, for example `http://127.0.0.1:7000`.
- `BASEDCODE_AI_API_TOKEN`: Scoped API token created in BasedCode AI Settings > Integrations > Add Integration > Claude Agent.

If either value is missing, do not guess credentials. Tell the user to create a Claude Agent token in BasedCode AI Settings and expose both values to the terminal session.
...
- **Calendar event ("meeting at 3pm", "dentist Tuesday 10am")** → calendar event. Use for scheduled time blocks, meetings, appointments, recurring schedules. These show up on the calendar grid; reminders for them are configured separately in BasedCode AI settings.
...
- All BasedCode AI data access MUST go through the scoped HTTP API under `/api/codex/*` (the canonical scope-gated agent API, shared by all agent integrations).
...
- Do not use SSH, Docker, direct Python imports, SQLite queries, MCP internals, browser cookies, or local files to read/write BasedCode AI user data.
...
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py todos list
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py todos add "Follow up"
```
...
```bash
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py emails list 5
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py emails read UID
```
...
```bash
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py GET /api/codex/memory
python3 ~/.claude/skills/basedcode-ai/scripts/basedcode_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
If you are about to reach the BasedCode AI host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass BasedCode AI Settings and token scopes. Ask the user to enable the relevant Claude Agent tool toggle instead.
