---
name: basedcode-ai
description: Use when the user asks Codex to read or write BasedCode AI data from a terminal Codex session through the scoped Codex Agent API. Requires BASEDCODE_AI_URL and BASEDCODE_AI_API_TOKEN.
---

# BasedCode AI

Use this skill when a user asks to interact with BasedCode AI from Codex.
...
- `BASEDCODE_AI_API_TOKEN`: Scoped API token created in BasedCode AI Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in BasedCode AI Settings and expose both values to the terminal session.
...
- All BasedCode AI data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
```bash
python3 integrations/codex/scripts/basedcode_api.py capabilities
python3 integrations/codex/scripts/basedcode_api.py todos list
python3 integrations/codex/scripts/basedcode_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/basedcode_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 integrations/codex/scripts/basedcode_api.py emails list 5
python3 integrations/codex/scripts/basedcode_api.py emails read UID
```
...
```bash
python3 integrations/codex/scripts/basedcode_api.py GET /api/codex/memory
python3 integrations/codex/scripts/basedcode_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
If you are about to reach the BasedCode AI host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass BasedCode AI Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
