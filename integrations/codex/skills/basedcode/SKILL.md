---
name: basedcode
description: Use when the user asks Codex to read or write Based code data from a terminal Codex session through the scoped Codex Agent API. Requires BASEDCODE_URL and BASEDCODE_API_TOKEN.
---

# Based code

Use this skill when a user asks to interact with Based code from Codex.
...
- `BASEDCODE_API_TOKEN`: Scoped API token created in Based code Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in Based code Settings and expose both values to the terminal session.
...
- All Based code data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
If you are about to reach the Based code host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass Based code Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
