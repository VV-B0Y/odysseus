...
# Override at deploy time via `BASEDCODE_AI_FALLBACK_OWNER` env var. In a real
# multi-user install set `BASEDCODE_AI_SINGLE_USER=0` so unauthenticated requests
# are rejected instead of silently writing to this address.
import os as _os
FALLBACK_OWNER = _os.environ.get("BASEDCODE_AI_FALLBACK_OWNER", "owner@localhost")
_SINGLE_USER_MODE = _os.environ.get("BASEDCODE_AI_SINGLE_USER", "1") != "0"
    ...
    async def export_ics(request: Request, cal_id: str):
        ...
        try:
            ...
            lines = [
                "BEGIN:VCALENDAR",
                "VERSION:2.0",
                "PRODID:-//BasedCode AI//Calendar//EN",
