...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When BasedCode AI runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends BasedCode AI' own interpreter directory to PATH. If BasedCode AI itself is
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'BASEDCODE_AI_USER_SHELL="${SHELL:-}"',
        'if [ -n "$BASEDCODE_AI_USER_SHELL" ] && [ -x "$BASEDCODE_AI_USER_SHELL" ]; then',
        '  BASEDCODE_AI_USER_PATH="$("$BASEDCODE_AI_USER_SHELL" -ic \'printf "__BASEDCODE_AI_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__BASEDCODE_AI_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$BASEDCODE_AI_USER_PATH" ]; then export PATH="$BASEDCODE_AI_USER_PATH:$PATH"; fi',
        'fi',
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by BasedCode AI.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$BASEDCODE_AI_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $BASEDCODE_AI_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        runner_lines.append('  exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('  exit "$BASEDCODE_AI_PREFLIGHT_EXIT"')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('BASEDCODE_AI_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $BASEDCODE_AI_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $BASEDCODE_AI_CMD_EXIT ==="; exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('echo ""; echo "=== Process exited with code $BASEDCODE_AI_CMD_EXIT ==="')
        runner_lines.append('exit "$BASEDCODE_AI_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[basedcode-ai] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('      _basedcode_ai_has_cudart() {')
    ...
    runner_lines.append('      }')
    runner_lines.append('      if _basedcode_ai_has_cudart; then')
    runner_lines.append('        echo "[basedcode-ai] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('        echo "[basedcode-ai] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[basedcode-ai]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[basedcode-ai]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    else')
    runner_lines.append('      echo "[basedcode-ai] WARNING: no HIP/CUDA toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[basedcode-ai]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[basedcode-ai]   Install ROCm for AMD GPUs or vLLM/CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
...
def _llama_cpp_rebuild_cmd() -> str:
    ...
    return (
        ...
        'echo "[basedcode-ai] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\basedcode-ai-sessions"
