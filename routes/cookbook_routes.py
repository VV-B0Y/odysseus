...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[basedcode-ai] HF token: applied"; '
    'else '
    'echo "[basedcode-ai] HF token: NOT SET — gated/private models will be denied. '
    'Add one in BasedCode AI Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "basedcode-ai-cookbook", "-f", str(key_path),
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When BasedCode AI runs from a venv (e.g. native macOS install), put its bin
        ...
        if remote and is_windows:
            ...
            ps_lines = []
            ps_lines.append('$sessionDir = "$env:TEMP\\basedcode-ai-sessions"')
            ...
            launch_ps = (
                "$sd = \\\"$env:TEMP\\basedcode-ai-sessions\\\"; "
    ...
    async def model_cached(request: Request, host: str | None = None, model_dir: str | None = None, ssh_port: str | None = None, platform: str | None = None):
        ...
        if host:
            ...
            # LOCAL scan: use sys.executable (the venv Python BasedCode AI is already
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('BASEDCODE_AI_PREFLIGHT_EXIT=""')
            # Put BasedCode AI's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('    BASEDCODE_AI_PREFLIGHT_EXIT=127')
                runner_lines.append('  fi')
                runner_lines.append('fi')
            elif "ollama" in req.cmd:
                ...
                runner_lines.append(f'BASEDCODE_AI_OLLAMA_HOST={_bash_squote(_ollama_host)}')
                runner_lines.append(f'BASEDCODE_AI_OLLAMA_PORT="{_ollama_port}"')
                runner_lines.append('BASEDCODE_AI_OLLAMA_URL=""')
                runner_lines.append('for _ody_ollama_try in $(seq 1 20); do')
                runner_lines.append('  for _ody_ollama_port in "$BASEDCODE_AI_OLLAMA_PORT" 11434; do')
                ...
                runner_lines.append('        BASEDCODE_AI_OLLAMA_URL="$_ody_ollama_url"')
                runner_lines.append('        BASEDCODE_AI_OLLAMA_PORT="$_ody_ollama_port"')
                runner_lines.append('        break 3')
                runner_lines.append('      fi')
                runner_lines.append('    done')
                runner_lines.append('  done')
                runner_lines.append('  [ "$_ody_ollama_try" -eq 1 ] && echo "[basedcode-ai] Waiting for an existing Ollama API on ports ${BASEDCODE_AI_OLLAMA_PORT}/11434..."')
                runner_lines.append('  sleep 1')
                runner_lines.append('done')
                runner_lines.append('if [ -n "$BASEDCODE_AI_OLLAMA_URL" ]; then')
                runner_lines.append('  if [ "$BASEDCODE_AI_OLLAMA_PORT" != "' + _ollama_port + '" ]; then')
                runner_lines.append('    echo "[basedcode-ai] Selected Ollama port ' + _ollama_port + ' was not reachable; using running Ollama on port ${BASEDCODE_AI_OLLAMA_PORT}."')
                runner_lines.append('  fi')
                runner_lines.append('  echo "[basedcode-ai] Ollama API ready on port ${BASEDCODE_AI_OLLAMA_PORT}: ${BASEDCODE_AI_OLLAMA_URL}"')
                runner_lines.append('  echo "[basedcode-ai] This task is monitoring an existing Ollama server; stopping it here will not stop an external Docker/system service."')
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('if ! command -v ollama &>/dev/null; then')
                runner_lines.append('  echo "ERROR: Ollama not found and no Ollama API is reachable on 127.0.0.1, localhost, or host.docker.internal (ports ${BASEDCODE_AI_OLLAMA_PORT}/11434)."')
                ...
                runner_lines.append('  exec bash -i')
                runner_lines.append('fi')
                runner_lines.append('BASEDCODE_AI_OLLAMA_URL="http://${BASEDCODE_AI_OLLAMA_HOST}:${BASEDCODE_AI_OLLAMA_PORT}"')
                if remote and _ollama_host in ("0.0.0.0", "::"):
                    runner_lines.append('echo "[basedcode-ai] WARNING: remote Ollama will bind to ${BASEDCODE_AI_OLLAMA_HOST}:${BASEDCODE_AI_OLLAMA_PORT} so BasedCode AI can reach it from this host."')
                    runner_lines.append('echo "[basedcode-ai] Ollama has no built-in authentication; expose this only on a trusted LAN/VPN or provide an explicit OLLAMA_HOST with your own access controls."')
                runner_lines.append('echo "Starting ollama server on ${BASEDCODE_AI_OLLAMA_HOST}:${BASEDCODE_AI_OLLAMA_PORT}..."')
                runner_lines.append('OLLAMA_HOST="${BASEDCODE_AI_OLLAMA_HOST}:${BASEDCODE_AI_OLLAMA_PORT}" ollama serve')
                ...
                runner_lines.append('  BASEDCODE_AI_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
                ...
                runner_lines.append('  BASEDCODE_AI_PREFLIGHT_EXIT=127')
                runner_lines.append('elif ! BASEDCODE_AI_SGLANG_IMPORT_ERROR="$(python3 -c "import sglang" 2>&1)"; then')
                runner_lines.append('  echo "ERROR: SGLang is installed but failed to import."')
                runner_lines.append('  printf "%s\\n" "$BASEDCODE_AI_SGLANG_IMPORT_ERROR"')
                runner_lines.append('  BASEDCODE_AI_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
            elif "scripts/diffusion_server.py" in req.cmd or ".diffusion_server.py" in req.cmd:
                runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
                runner_lines.append('if ! BASEDCODE_AI_DIFFUSION_IMPORT_ERROR="$(python3 -c "import torch, diffusers" 2>&1)"; then')
                runner_lines.append('  echo "ERROR: Diffusion serving requires PyTorch + diffusers."')
                runner_lines.append('  printf "%s\\n" "$BASEDCODE_AI_DIFFUSION_IMPORT_ERROR"')
                runner_lines.append('  BASEDCODE_AI_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\basedcode-ai-sessions | Out-Null; "
