                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in BasedCode AI (if public URL is configured).
                                    ...
                                    alert_body = (
                                        ...
                                        f"Reason: {reason}\n\n"
                                        + (f"Open in BasedCode AI: {open_url}\n\n" if open_url else "")
                                        + f"---\n"
                                        f"From: {sender}\n"
                                        f"Subject: {subject}\n"
                                        f"Date: {msg.get('Date','')}\n\n"
                                    ...
                                    # HTML alternative with a clickable "Open in BasedCode AI" button
                                    import html as _h
...
async def _scheduled_email_poller():
    ...
    also exposed via the `basedcode-ai-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `BASEDCODE_AI_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `basedcode-ai-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("BASEDCODE_AI_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `BASEDCODE_AI_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (BASEDCODE_AI_INPROCESS_POLLERS=0); "
            "drive `basedcode-ai-mail poll-scheduled` externally."
        )
        return
    import asyncio
