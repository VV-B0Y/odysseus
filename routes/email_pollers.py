                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Based code (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Based code" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "basedcode_kind" if "basedcode_kind" in cols else "'scheduled' AS basedcode_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                basedcode_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-BasedCode-Origin"] = "basedcode-ui"
                outer["X-BasedCode-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", basedcode_kind or "scheduled")[:64]
                outer["X-BasedCode-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `basedcode-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `BASEDCODE_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `basedcode-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("BASEDCODE_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `BASEDCODE_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (BASEDCODE_INPROCESS_POLLERS=0); "
            "drive `basedcode-mail poll-scheduled` externally."
        )
        return
    import asyncio
