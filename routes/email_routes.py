...
BASEDCODE_MAIL_ORIGIN = "basedcode-ui"
...
def _apply_basedcode_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-BasedCode-Origin"] = BASEDCODE_MAIL_ORIGIN
    if kind:
        msg["X-BasedCode-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-BasedCode-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Based code marker header, but include the subject
                # fallback too. The fallback uses a distinct Based code prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                reminder_uids = set()
                for query in (
                    f'(HEADER X-BasedCode-Kind "reminder"{from_clause})',
                    f'(HEADER X-Odysseus-Kind "reminder"{from_clause})',
                    f'(SUBJECT "Reminder (Based code):"{from_clause})',
                    f'(SUBJECT "Reminder (Odysseus):"{from_clause})',
                ):
                    st_rem, data_rem = _imap_uid_search(conn, query)
                    if st_rem == "OK" and data_rem and data_rem[0]:
                        reminder_uids.update(data_rem[0].split())
                data = [b" ".join(sorted(reminder_uids, key=lambda x: int(x) if x.isdigit() else 0))]
                status = "OK"
    ...
    @router.delete("/basedcode/reminders")
    async def delete_basedcode_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Based code reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-BasedCode-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(HEADER X-Odysseus-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Based code):")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Odysseus):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Based code):")})'))
                            # Legacy reminders created before the Based code
            ...
            logger.error(f"delete_basedcode_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", basedcode_kind=None, basedcode_ref=None,
    ):
        ...
        _apply_basedcode_headers(outer, basedcode_kind or "scheduled", basedcode_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, basedcode_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("basedcode_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="basedcode.local")

        if req.in_reply_to:
            outer["In-Reply-To"] = req.in_reply_to
        if req.references:
            outer["References"] = req.references
        if req.basedcode_kind:
            _apply_basedcode_headers(outer, req.basedcode_kind)
