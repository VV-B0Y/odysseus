...
BASEDCODE_AI_MAIL_ORIGIN = "basedcode-ai"
...
def _apply_basedcode_ai_headers(msg, kind: str | None = None, ref_id: str | None = None):
    ...
    if ref_id:
        msg["X-BasedCode AI-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the BasedCode AI marker header, but include the subject
                # fallback too. The fallback uses a distinct BasedCode AI prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-BasedCode AI-Kind "reminder" SUBJECT "Reminder (BasedCode AI):"{from_clause})',
                )
    ...
    async def delete_basedcode_ai_reminder_emails(
        ...
        """Delete email messages stamped as BasedCode AI reminders."""
        if account_id:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-BasedCode AI-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (BasedCode AI):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (BasedCode AI):")})'))
                            # Legacy reminders created before the BasedCode AI
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="basedcode-ai.local")
