...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the BasedCode AI container by design. The image ships no "
...
def _package_pip_update_status(pkg: dict, probe: dict | None = None) -> PackageUpdateStatus:
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(False, "Update this system dependency outside BasedCode AI.")
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside BasedCode AI.",
        )
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "basedcode-ai-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f"BASEDCODE_AI_USER_SHELL=\"${{SHELL:-}}\"\n"
        f"if [ -n \"$BASEDCODE_AI_USER_SHELL\" ] && [ -x \"$BASEDCODE_AI_USER_SHELL\" ]; then\n"
        f"  BASEDCODE_AI_USER_PATH=\"$(\"$BASEDCODE_AI_USER_SHELL\" -ic 'printf \"__BASEDCODE_AI_PATH__%s\\n\" \"$PATH\"' 2>/dev/null | sed -n 's/^__BASEDCODE_AI_PATH__//p' | tail -n 1 || true)\"\n"
        f"  if [ -n \"$BASEDCODE_AI_USER_PATH\" ]; then export PATH=\"$BASEDCODE_AI_USER_PATH:$PATH\"; fi\n"
        f"fi\n"
