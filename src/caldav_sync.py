...
def _private_caldav_allowed() -> bool:
    return os.environ.get("BASEDCODE_ALLOW_PRIVATE_CALDAV", "0").lower() in {"1", "true", "yes"}
...
def _validate_caldav_ip(host: str) -> None:
    ...
    if ip.is_private and not _private_caldav_allowed():
        raise ValueError("Private CalDAV IPs require BASEDCODE_ALLOW_PRIVATE_CALDAV=1")
