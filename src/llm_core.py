...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Based code' canonical OpenAI-style messages to native Ollama /api/chat.

    Based code carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/VV-B0Y/BasedCode")
        h.setdefault("X-OpenRouter-Title", "Based code")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Based code-only metadata before sending messages to providers.
