...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt BasedCode AI' canonical OpenAI-style messages to native Ollama /api/chat.

    BasedCode AI carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/pewdiepie-archdaemon/basedcode-ai")
        h.setdefault("X-OpenRouter-Title", "BasedCode AI")
    return h
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip BasedCode AI-only metadata before sending messages to providers.
