#!/bin/bash
# Based code — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Based code needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Based code
...
# Shell overrides (BASEDCODE_PORT / BASEDCODE_HOST) take top priority, then the
# legacy ODYSSEUS_* names, then .env values (APP_PORT / APP_BIND), then
# built-in defaults.
PORT="${BASEDCODE_PORT:-${ODYSSEUS_PORT:-${APP_PORT:-7860}}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${BASEDCODE_HOST:-${ODYSSEUS_HOST:-${APP_BIND:-127.0.0.1}}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ Based code quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
  ...
  echo "    BASEDCODE_PORT=7900 ./start-macos.sh"
  exit 1
fi
...
echo "▶ Preparing Based code…"
BASEDCODE_SKIP_RUN_HINT=1 ./venv/bin/python setup.py

# 5. Launch. Bind to loopback by default; opt into LAN/Tailscale with
#    BASEDCODE_HOST=0.0.0.0.
URL_HOST="$HOST"
...
# BASEDCODE_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "${BASEDCODE_NO_OPEN:-${ODYSSEUS_NO_OPEN:-}}" ] && command -v open >/dev/null 2>&1; then
  (
    for _ in $(seq 1 90); do
      if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
        printf '\n'
        printf '  ┌────────────────────────────────────────────┐\n'
        printf '  │  ✓ Based code is ready — opening your browser  │\n'
        printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Based code — it will open in your browser at $URL"
