// ============================================
// BasedCode AI UI — Main Application Orchestrator
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__basedcode-aiDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'BasedCode AI Chat';
  ...
  if (_opener) window._basedcode-aiRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('basedcode-ai-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'basedcode-ai-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message BasedCode AI...');
      }
  ...
  const UI_VIS_KEY = 'basedcode-ai-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'basedcode-ai-toolbar-visibility';
    try {
...
// ============================================
function startBasedCode AIApp() {
  if (window.__basedcode-aiAppStarted) return;
  window.__basedcode-aiAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('basedcode-ai-doc-open-' + _curSession) === '1') {
  ...
  window._basedcode-aiBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._basedcode-aiRouteOpener) {
          try { window._basedcode-aiRouteOpener(); } catch (_) {}
          window._basedcode-aiRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startBasedCode AIApp, { once: true });
} else {
  startBasedCode AIApp();
}
