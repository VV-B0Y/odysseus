...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__basedcode-aiActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'basedcode-ai-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'basedcode-ai-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__basedcode-aiActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('basedcode-ai-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('basedcode-ai-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'basedcode-ai-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('basedcode-ai-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('basedcode-ai-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('basedcode-ai-suggestions-' + docId);
      if (!raw) return;
