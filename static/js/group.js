...
const GROUP_STATE_KEY = 'basedcode-ai-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('basedcode-ai-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('basedcode-ai-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          // Text delta (BasedCode AI format)
