  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('basedcode-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('basedcode-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('basedcode-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('basedcode-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('basedcode-char-sessions', JSON.stringify(charSessions));
