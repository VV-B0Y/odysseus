  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('basedcode-ai-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('basedcode-ai-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('basedcode-ai-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('basedcode-ai-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('basedcode-ai-char-sessions', JSON.stringify(charSessions));
