...
let _sortMode = Storage.get('basedcode-ai-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'BasedCode AI Chat';
...
const FOLDER_STATE_KEY = 'basedcode-ai-folder-state';
const FOLDER_ORDER_KEY = 'basedcode-ai-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to BasedCode AI kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'BasedCode AI Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('basedcode-ai-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('basedcode-ai-session-sort', mode);
  else Storage.remove('basedcode-ai-session-sort');
  renderSessionList();
}
