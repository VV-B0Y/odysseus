...
let _sortMode = Storage.get('basedcode-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Based code Chat';
...
const FOLDER_STATE_KEY = 'basedcode-folder-state';
const FOLDER_ORDER_KEY = 'basedcode-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Based code kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Based code Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('basedcode-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('basedcode-session-sort', mode);
  else Storage.remove('basedcode-session-sort');
  renderSessionList();
}
