...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "BASEDCODE_AI_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $BASEDCODE_AI_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$BASEDCODE_AI_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_basedcode_ai_has_cudart' in script
    ...
    assert script.index('_basedcode_ai_has_cudart') < script.index('DGGML_CUDA=ON')
