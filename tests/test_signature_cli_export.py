...
def _load_signature_cli(monkeypatch):
    ...
    path = Path(__file__).resolve().parent.parent / "scripts" / "basedcode-signature"
    loader = importlib.machinery.SourceFileLoader("basedcode_signature_cli_under_test", str(path))
